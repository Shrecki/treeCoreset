...
public:
    void operator()(ArgumentList outputs, ArgumentList inputs) {
        try{
            ...
            // Get centroids
            std::vector<std::vector<double>*>* received_representatives = ClientMessaging::requestRepresentatives(socket, 50000);

            int n_centroids = received_representatives->size();
            int dimension = received_representatives->at(0)->size();
            ...
            for(int i=0; i < n_transmit; ++i){
                TypedArray<double> tmpCentroid = f.createArray<double>({1, dimension}, received_representatives->at(i)->data(), received_representatives->at(i)->data() + received_representatives->at(i)->size()); //std::move(received_representatives->at(i)->data());//f.createArray<double>({dimension,1});
                outputs[i] = tmpCentroid;
            }

            ClientMessaging::free_vector(received_representatives);
